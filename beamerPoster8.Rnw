...
\includegraphics[width=10cm]{Ci-map-CIA}
\end{wrapfigure} 
%------------------------------------------
...
%------------------------------------------
{\large
Logistic regression models were used to estimate the odds ratios of low HDL-C ($\le$40 mg/dL) for a one unit change in BMI z-scores at age 5. Analyses stratified by sex of child and multiple imputation methods handled missing values. 
