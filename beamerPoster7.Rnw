...
\begin{itemize}
  ...
  \item Most evidence corresponds to European populations with cross sectional data. This research represents a Chilean sample typifying the epidemiological transition, moving from infectious disease and malnutrition to chronic disease and an obesogenic environment in the 1980s.
  \end{itemize}
\end{block}
...
\end{wrapfigure} 
%------------------------------------------
{\normalsize Approximately 1,100 full-term infants $\ge$ 3 kg from Santiago, Chile were recruited from 1991 to 1996. Participants were from lower/middle class and ethnically mixed American Indian and Spanish descent families. 677 completed CVD risk factor assessments at 16-17 years.
%\vskip5ex
\par
%------------------------------------------
}
...
\normalsize 
\begin{itemize}
  \item Logistic regression models were used to estimate the odds ratios of low HDL-C ($\le$40 mg/dL) for a one unit change in BMI z-scores at age 5. Potential confounders included sex of child, child height, SES index of PCA on Graffar scale, gestational age and maternal age.
  \item Multiple imputation methods handled missing values. 
  \item We stratified regression models by sex of child, a significant effect modifier.
...
\begin{block}{Results, Descriptive}
...
latex(summary(sub.i2.hmisc$"Sex of child" ~ ., 
               data=sub.i2.hmisc[c(5,7,6,8,9,10,11,13,4)], # taking out maternal education for now
...
\begin{table}[H]
\centering
\caption{\normalsize Table 2. Odds ratios of low HDL-C groups for a one unit change in BMI z-score by gender and covariate adjustment status (95\% CI)}
\label{t4}
\normalsize
...
\begin{figure}
\centering
\caption{\normalsize Figure 1. Association between BMI z-scores at age 5 and HDL-C at 17 years}
\vskip-2ex
...
\begin{itemize}
  ...
  \item \normalsize \textbf{Sex differences}: Girls demonstrate a lower odds ratio of low HDL-C for a one unit increase in BMI than boys.
    \begin{itemize}
      \item Association between BMI and HDL-C statistically significant for boys [adjusted OR = 1.51, p-value$<$0.001] but not girls [adjusted OR = 1.18, p-value=0.17], with or without adjustment.
    \end{itemize}
...
\begin{block}{Summary}
  \begin{itemize}
    \item \normalsize Results support continued focus on weight in childhood to mitigate development of adverse cardiovascular risk factors.
    \item \normalsize Increasing BMI z-score is a stronger predictor of adverse HDL-C levels in boys compared to girls. These sex differences in risk factors in childhood can inform prevention efforts.
    \end{itemize}
\end{block}

\begin{block}{Future Efforts}
  \begin{itemize}
        \item \normalsize Replicate the analysis in a new sample. Differing associations between BMI and HDL-C by sex of child may be a chance finding.
%        \item Investigate the relationship between child weight trajectories and HDL-C.
        \item \normalsize Consider other measures of adiposity and other risk factors associated with adiposity in childhood.
  \end{itemize}
\end{block}

\begin{block}{Acknowledgements}
  \normalsize We thank all study participants for their generosity in time and support for this ongoing project. \small The Santiago Longitudinal Cohort Study was supported by NIH: NHLBI R01 HL088530 (PI: Gahagan) and NICHD R01 HD33487 (PIs: Gahagan and Lozoff).
\end{block}
